	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(label.equalsIgnoreCase("db") || label.equalsIgnoreCase("database")){
			...
			if(args.length == 0){
				...
				sender.sendMessage("§c/db addTokens (player) (amount)");
				sender.sendMessage("§c/db setTokens (player) (amount)");
				sender.sendMessage("§c/db setChests (player) (amount)");
				sender.sendMessage("§c/db addChests (player) (amount)");
				...
				if(args[0].equalsIgnoreCase("getters")){
					...
					sender.sendMessage("§c/db getTokens (player)");
					sender.sendMessage("§c/db getChests (player)");
					sender.sendMessage("§c--------------------------------");
					return true;
				}
				else{
				...
				if(args[0].equalsIgnoreCase("getLevel")){
					...
					sender.sendMessage("§cThe K/D of the player " + target.getName() + " is: " + api.getKD(target.getName()));
				}
				...
				else if(args[0].equalsIgnoreCase("getChests")){
					sender.sendMessage("§cThe chests of the player " + target.getName() + ": " + cm.getChests(target.getName()));
				}
				else if(args[0].equalsIgnoreCase("getTokens")){
					sender.sendMessage("§cThe tokens of the player " + target.getName() + ": " + cm.getTokens(target.getName()));
				}
				...
				if(args[0].equalsIgnoreCase("addBalance")){
				...
				else if(args[0].equalsIgnoreCase("addChests")){
				...
				else if(args[0].equalsIgnoreCase("setChests")){
					...
					cm.setTokens(target.getName(), amount);
					sender.sendMessage("§cYou set the tokens of the player " + target.getName() + " to: " + amount);
					return true;
					
				}
				else if(args[0].equalsIgnoreCase("addTokens")){
					
					cm.addTokens(target.getName(), amount);
					sender.sendMessage("§cYou added " + amount + " tokens to the player " + target.getName());
					return true;
