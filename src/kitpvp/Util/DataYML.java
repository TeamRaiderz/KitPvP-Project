...
public class DataYML {
	...
	if(pDataFile == null){
		pDataFile = new File(Bukkit.getPluginManager().getPlugin("KitPvP").getDataFolder(), "data.yml");
	}
	
	pData = YamlConfiguration.loadConfiguration(pDataFile);
	
	InputStream defConfigStream = Bukkit.getPluginManager().getPlugin("KitPvP").getResource("data.yml");
